<button data-id="{{ $product->id }}" data-name="{{ addslashes($product->name) }}" data-price="{{ $product->price }}" data-stock="{{ $product->stock }}" data-search="{{ strtolower($product->name.' '.$product->sku.' '.($product->category?->name ?? '')) }}" data-category="cat-{{ $product->category_id }}" class="product-card" {{ $product->stock <= 0 ? 'disabled' : '' }}>
    ...
    <div class="price-row" style="display:flex;justify-content:space-between;align-items:end;margin-top:8px;gap:6px;flex-wrap:wrap;"><b class="price-val" style="font-size:13px;color:#0F6E56;white-space:nowrap;">Rp {{ number_format($product->price,0,',','.') }}</b><span class="stock-label" style="font-size:11px;color:{{ $product->stock <= 0 ? '#dc2626' : '#94a3b8' }};font-weight:{{ $product->stock <= 0 ? '700' : '400' }};white-space:nowrap;">{{ $product->stock <= 0 ? 'Stok habis' : 'Stok '.$product->stock }}</span></div>
</button>
