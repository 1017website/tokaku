...
.product-card{background:#fff;border:1.5px solid #edf2f7;border-radius:16px;padding:12px;text-align:left;cursor:pointer;min-height:142px;transition:.15s;font-family:Inter,sans-serif;overflow:hidden;min-width:0}
...
<div class="pos-wrap">
    ...
    <div class="pos-panel cart-sticky">
        ...
        <div style="padding:14px 16px;border-top:1px solid #f8fafc;display:flex;flex-direction:column;gap:9px;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;"><span style="font-size:13px;color:#64748b;">Diskon</span><div style="display:flex;align-items:center;gap:5px;"><span style="font-size:12px;color:#9ca3af;">Rp</span><input type="text" inputmode="numeric" id="discountInput" value="0" oninput="formatThousand(this);recalculate()" style="width:92px;text-align:right;border:1.5px solid #e2e8f0;border-radius:8px;padding:6px 8px;font-size:13px;"></div></div>
            @if($taxEnabled)
...
function recalculate(){ var sub=0; Object.values(cart).forEach(function(i){sub+=i.price*i.qty;}); var disc=parseNum('discountInput'); var afterDisc=Math.max(0,sub-disc); var tax=calcTax(afterDisc); var tot=afterDisc+tax; var paid=parseNum('paidInput'); document.getElementById('subtotalDisplay').textContent=fmt(sub); var taxEl=document.getElementById('taxDisplay'); if(taxEl) taxEl.textContent=fmt(tax); document.getElementById('totalDisplay').textContent=fmt(tot); document.getElementById('changeDisplay').textContent=fmt(Math.max(0,paid-tot)); }
...
function showConfirmModal(){ if(!Object.keys(cart).length){alert('Keranjang masih kosong!');return;} var sub=0; Object.values(cart).forEach(function(i){sub+=i.price*i.qty;}); var disc=parseNum('discountInput'), afterDisc=Math.max(0,sub-disc), tax=calcTax(afterDisc), total=afterDisc+tax, paid=parseNum('paidInput'); if(paymentMethod==='cash' && paid<total){alert('Jumlah bayar kurang dari total!');return;} var html=''; Object.values(cart).forEach(function(i){html+='<div style="display:flex;justify-content:space-between;gap:10px;padding:10px 0;border-bottom:1px solid #f1f5f9;"><div><b style="font-size:13.5px;color:#0f172a;">'+escapeHtml(i.name)+'</b><p style="font-size:12px;color:#64748b;">'+fmt(i.price)+' × '+i.qty+'</p></div><b style="font-size:13px;color:#0f172a;">'+fmt(i.price*i.qty)+'</b></div>';}); document.getElementById('confirmItemList').innerHTML=html; var taxLine = (TAX_ENABLED && tax>0) ? '<div style="display:flex;justify-content:space-between;"><span>{{ $taxName }} ('+TAX_RATE+'%)</span><b>'+fmt(tax)+'</b></div>' : ''; document.getElementById('confirmSummary').innerHTML='<div style="display:flex;justify-content:space-between;"><span>Subtotal</span><b>'+fmt(sub)+'</b></div><div style="display:flex;justify-content:space-between;"><span>Diskon</span><b>'+fmt(disc)+'</b></div>'+taxLine+'<div style="display:flex;justify-content:space-between;border-top:1px solid #e2e8f0;padding-top:8px;"><b>Total</b><b style="color:#0F6E56;">'+fmt(total)+'</b></div>'; document.getElementById('modalKonfirmasi').style.display='flex'; }
