...
<div style="background:#f0fdf6;border:1px solid #bbf7d2;border-radius:12px;padding:10px 16px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;gap:8px;">
    <span style="font-size:13px;font-weight:600;color:#15803d;">Shift aktif &middot; {{ $activeShift->user->name ?? 'Kasir' }} &middot; dibuka {{ $activeShift->opened_at->translatedFormat('d M Y, H:i') }}</span>
...
<div class="pos-wrap">
    <div class="pos-panel" style="display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:16px 18px;border-bottom:1px solid #f8fafc;">
            ...
            <div class="category-tabs" id="categoryTabs">
                @if(isset($topProducts) && $topProducts->count())
                    <button type="button" class="cat-btn active" data-category="top">Top Order</button>
                @endif
                @foreach($categories as $category)
                    <button type="button" class="cat-btn" data-category="cat-{{ $category->id }}">{{ $category->name }}</button>
                @endforeach
                <button type="button" class="cat-btn {{ (isset($topProducts) && $topProducts->count()) ? '' : 'active' }}" data-category="all">Semua Produk</button>
            </div>
        </div>

        @if(isset($topProducts) && $topProducts->count())
        <div class="product-grid product-section" data-section="top">
        ...
        <div class="product-grid product-section" data-section="all" style="{{ (isset($topProducts) && $topProducts->count()) ? 'display:none;' : '' }}">
    ...
    <div class="pos-panel cart-sticky">
        ...
        <div style="padding:14px 16px;border-top:1px solid #f8fafc;display:flex;flex-direction:column;gap:9px;">
            ...
            <div id="promoRow" style="display:none;justify-content:space-between;align-items:center;"><span style="font-size:13px;color:#0F6E56;font-weight:600;">🎉 <span id="promoLabel">Promo</span></span><b id="promoDisplay" style="font-size:13px;color:#0F6E56;">- Rp 0</b></div>
            <div style="display:flex;justify-content:space-between;align-items:center;"><span style="font-size:13px;color:#64748b;">Diskon Manual</span><div style="display:flex;align-items:center;gap:5px;"><span style="font-size:12px;color:#9ca3af;">Rp</span><input type="text" inputmode="numeric" id="discountInput" value="0" oninput="formatThousand(this);recalculate()" style="width:92px;text-align:right;border:1.5px solid #e2e8f0;border-radius:8px;padding:6px 8px;font-size:13px;"></div></div>
            @if($taxEnabled)
            <div id="taxRow" style="display:flex;justify-content:space-between;"><span style="font-size:13px;color:#64748b;">{{ $taxName }} ({{ rtrim(rtrim(number_format($taxRate,2,',','.'),'0'),',') }}%)</span><b id="taxDisplay" style="font-size:13px;color:#0f172a;">Rp 0</b></div>
            @endif
            ...
            <div><label style="display:block;font-size:12px;font-weight:700;color:#64748b;margin-bottom:5px;">Jumlah Bayar</label><input type="text" inputmode="numeric" id="paidInput" placeholder="0" oninput="formatThousand(this);recalculate()" class="form-input"></div>
...
// Ambil nilai numeric murni dari input ber-separator (buang semua titik/non-digit)
function parseNum(id){ var el=document.getElementById(id); if(!el) return 0; return parseInt((el.value||'').replace(/[^0-9]/g,''),10)||0; }
// Format isi input jadi ber-titik ribuan saat diketik
function formatThousand(el){ var digits=(el.value||'').replace(/[^0-9]/g,''); el.value = digits ? parseInt(digits,10).toLocaleString('id-ID') : ''; }
...
function renderCart(){ var c=document.getElementById('cartItems'), badge=document.getElementById('cartBadge'), clr=document.getElementById('btnClear'), keys=Object.keys(cart); if(!keys.length){ c.innerHTML='<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:34px 0;text-align:center;color:#94a3b8;font-size:13px;">Keranjang kosong<br><span style="font-size:12px;color:#cbd5e1;margin-top:4px;">Klik produk untuk menambahkan</span></div>'; badge.style.display='none'; clr.style.display='none'; promoState={discount:0,promo_id:null,label:null}; recalculate(); return;} badge.textContent=keys.length; badge.style.display='inline'; clr.style.display='block'; var html=''; keys.forEach(function(id){var i=cart[id]; html+='<div style="display:flex;align-items:center;gap:8px;padding:10px 0;border-bottom:1px solid #f8fafc;"><div style="flex:1;min-width:0;"><p style="font-size:13px;font-weight:700;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'+escapeHtml(i.name)+'</p><p style="font-size:12px;color:#0F6E56;font-weight:700;">'+fmt(i.price)+'</p></div><div style="display:flex;align-items:center;gap:5px;"><button onclick="changeQty(\''+id+'\',-1)" style="width:25px;height:25px;border-radius:7px;border:1.5px solid #e2e8f0;background:#fff;cursor:pointer;">−</button><b style="font-size:13px;min-width:20px;text-align:center;">'+i.qty+'</b><button onclick="changeQty(\''+id+'\',1)" style="width:25px;height:25px;border-radius:7px;border:1.5px solid #e2e8f0;background:#fff;cursor:pointer;">+</button></div><button onclick="removeItem(\''+id+'\')" style="background:none;border:none;color:#f43f5e;cursor:pointer;font-weight:800;">×</button></div>';}); c.innerHTML=html; recalculate(); scheduleFetchPromo(); }
function recalculate(){ var sub=0; Object.values(cart).forEach(function(i){sub+=i.price*i.qty;}); var manualDisc=parseNum('discountInput'); var promoDisc=promoState.discount||0; var disc=Math.max(manualDisc,promoDisc); var afterDisc=Math.max(0,sub-disc); var tax=calcTax(afterDisc); var tot=afterDisc+tax; var paid=parseNum('paidInput'); document.getElementById('subtotalDisplay').textContent=fmt(sub); var promoRow=document.getElementById('promoRow'); if(promoDisc>0){ promoRow.style.display='flex'; document.getElementById('promoLabel').textContent=promoState.label||'Promo'; document.getElementById('promoDisplay').textContent='- '+fmt(promoDisc); } else { promoRow.style.display='none'; } var taxEl=document.getElementById('taxDisplay'); if(taxEl) taxEl.textContent=fmt(tax); document.getElementById('totalDisplay').textContent=fmt(tot); document.getElementById('changeDisplay').textContent=fmt(Math.max(0,paid-tot)); }

// Ambil promo otomatis dari server berdasarkan isi keranjang (debounced).
function scheduleFetchPromo(){ clearTimeout(promoTimer); promoTimer=setTimeout(fetchPromo,250); }
async function fetchPromo(){
    var items=Object.values(cart).map(function(i){return {id:parseInt(i.id),qty:i.qty,price:i.price};});
    if(!items.length){ promoState={discount:0,promo_id:null,label:null}; recalculate(); return; }
    try{
        var res=await fetch('{{ route("tenant.promo.calculate") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')},body:JSON.stringify({items:items})});
        var data=await res.json();
        promoState={discount:parseInt(data.discount)||0,promo_id:data.promo_id||null,label:data.label||data.promo_name||null};
    }catch(e){ promoState={discount:0,promo_id:null,label:null}; }
    recalculate();
}
...
async function processTransaction(){ var btn=document.getElementById('btnKonfirmasi'), disc=parseNum('discountInput'), paid=parseNum('paidInput'); btn.disabled=true; btn.textContent='Memproses...'; try{ var res=await fetch('{{ route("tenant.kasir.proses") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').getAttribute('content')},body:JSON.stringify({items:Object.values(cart).map(function(i){return {id:parseInt(i.id),qty:i.qty};}),paid_amount:paid,payment_method:paymentMethod,discount:disc,tax_rate:(TAX_ENABLED?TAX_RATE:0),customer_id:document.getElementById('customerSelect').value || null})}); var data=await res.json(); if(data.success){ updateStockUI(data.stocks); closeConfirmModal(); window.lastTransactionId=data.transaction_id; document.getElementById('modalInvoice').textContent='Invoice '+(data.invoice_no || ('#'+data.transaction_id)); document.getElementById('btnStruk').href='/kasir/'+data.transaction_id+'/struk'; document.getElementById('btnPdf').href='/kasir/'+data.transaction_id+'/struk-pdf'; document.getElementById('cetakStatusKasir').textContent=''; document.getElementById('modalStruk').style.display='flex'; clearCart(); document.getElementById('paidInput').value=''; document.getElementById('discountInput').value='0'; document.getElementById('customerSelect').value=''; promoState={discount:0,promo_id:null,label:null}; recalculate(); } else alert(data.message || 'Terjadi kesalahan.'); } catch(e){ alert('Gagal terhubung ke server.'); console.error(e); } finally{ btn.disabled=false; btn.textContent='Konfirmasi & Bayar'; } }
...
function updateStockUI(stocks){
    ...
    Object.keys(stocks).forEach(function(id){
        var stok = parseInt(stocks[id]);
        // satu produk bisa muncul di beberapa section (Semua / Top Order)
