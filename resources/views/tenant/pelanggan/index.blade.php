            ...
            <div class="table-responsive overflow-x-auto">
                <table style="width:100%;border-collapse:collapse;min-width:500px;">
                    ...
                    <tr class="cust-row" data-name="{{ strtolower($c->name) }}" style="border-bottom:1px solid #f8fafc;transition:background 0.1s;" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background='#fff'">
                        <td data-label="Pelanggan" style="padding:12px 16px;">
                        ...
                        <td data-label="Kontak" style="padding:12px 16px;">
                            <p style="font-size:13px;color:#374151;">{{ $c->phone ?? '—' }}</p>
                            <p style="font-size:12px;color:#94a3b8;">{{ $c->email ?? '' }}</p>
                        </td>
                        <td data-label="Total Belanja" style="padding:12px 16px;font-size:13.5px;font-weight:700;color:#0F6E56;">Rp {{ number_format($c->total_spent,0,',','.') }}</td>
                        <td data-label="Transaksi" style="padding:12px 16px;font-size:13.5px;color:#374151;">{{ $c->total_transactions }}x</td>
                        <td data-label="Aksi" style="padding:12px 16px;">
                            <a href="{{ route('tenant.pelanggan.show', $c) }}" style="font-size:13px;color:#0F6E56;font-weight:500;text-decoration:none;">Detail</a>
                        </td>
                    </tr>
                    @empty
                    <tr><td data-empty colspan="5" style="padding:50px;text-align:center;color:#94a3b8;font-size:13.5px;">Belum ada pelanggan.</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
